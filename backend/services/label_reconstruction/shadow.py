...
Nothing in this repository currently calls this module from a production
request path (see ``backend/tests/test_label_reconstruction_not_wired_into_production.py``)
-- it exists so the shadow-logging behavior can be exercised and reviewed
before any router is wired to it.
"""
...
def reconstruct(raw_text: str, *, corruption_type_hint: Optional[List[str]] = None) -> LabelReconstructionResult:
