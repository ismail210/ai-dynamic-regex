...
def extract_geometry(
    ...
    *,
    dense_page_cap_strategy: str = "legacy_area",
) -> Dict[str, Any]:
    """Extract geometry objects from all pages of ``pdf_path``.

    ``dense_page_cap_strategy`` controls which sort key the >250-drawing
    cap uses to decide what to keep. Defaults to ``"legacy_area"``
    (unchanged production behavior — see the caution note below) so this
    parameter is purely additive; callers must opt in to
    ``"length_aware"`` to get the corrected behavior described in
    docs/geometry_graph_audit/08_prioritized_roadmap.md P0.1. Regardless
    of which strategy is active, every page where the cap triggers
    records an A/B diagnostic comparing what both strategies would have
    kept, so the current strategy's coverage loss is always observable.
    """
